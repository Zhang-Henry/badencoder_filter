...
from dataloader import get_dataloader,get_dataloader_bad
...
def create_bd(inputs, targets, opt):
    ...
    state_dict = torch.load('../../../../output/cifar10/stl10_backdoored_encoder/2023-12-25-20:38:31/unet_filter_200_trained.pt', map_location=torch.device('cpu'))
    net.load_state_dict(state_dict['model_state_dict'])
    # net=net.cuda().eval()
    bd_inputs=net(inputs)
    bd_inputs= clamp_batch_images(bd_inputs.cuda(),opt)
...
def eval(netC, test_dl1, test_dl2, opt):
    print(" Eval:")
    ...
    for batch_idx, (inputs, targets), (inputs2, targets2) in zip(range(len(test_dl1)), test_dl1, test_dl2):
        ...
        # )
        print(f'{batch_idx}/{len(test_dl1):}',end='\r')
...
def train(opt):
    ...
    test_dl1 = get_dataloader_bad()
    test_dl2 = get_dataloader_bad()
    ...
    if os.path.exists(ckpt_path):
        ...
        data = eval(netC, test_dl1, test_dl2, opt)
