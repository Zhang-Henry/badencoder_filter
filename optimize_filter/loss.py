...
#         # saturation_loss_hls = F.mse_loss(original_hls[:, 1, :, :], generated_hls[:, 1, :, :])

#         # 计算HSV的明度(V)损失和HLS的亮度(L)损失
#         value_loss = F.mse_loss(original_hsv[:, 2, :, :], generated_hsv[:, 2, :, :])
#         lightness_loss = F.mse_loss(original_hls[:, 2, :, :], generated_hls[:, 2, :, :])

#         # 综合损失
#         total_loss = hue_loss_hsv + saturation_loss_hsv + value_loss + lightness_loss
#         return total_loss
